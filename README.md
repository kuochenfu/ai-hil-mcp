...
6. build_firmware()
   → flash_firmware()
   → wait_for_pattern(pattern="boot|ready")  # confirm clean boot
```
...
| 2.2d | Build & Flash MCP Server — Rust rewrite (`std::process::Command` + `rmcp`) | ✅ | **Done** — hardware-verified on STM32WL55 and 2× ESP32-S3 boards. v2.0: `devices.toml` project registry, `board` param on all 5 tools (`list_projects` added), flash tool abstraction (openocd/esptool/idf/probe-rs), arch-aware size tool, ESP-IDF support (`build_tool="idf"`, `idf_path`). Full loop verified: build → esptool flash → Zenoh telemetry confirmed on both boards. |
...
| 4.4 | Multi-board support | ✅ | **Done** — `devices.toml` registry + aliases; `read_multi_log` concurrent N-board reads; `wait_for_pattern` cross-board event detection; verified on 2× Espressif boards |
...
| [2026-03-29](doc/2026-03-29.md) | All three Rust MCP servers upgraded to v2.0 with multi-board `devices.toml` registry; `build-flash-mcp` v2.0 hardware-verified: idf.py build + esptool flash on 2× ESP32-S3; Zenoh telemetry loop confirmed (s3node → s3gw relay) |
...
| [`doc/user-manual-serial-mcp.md`](doc/user-manual-serial-mcp.md) | serial-mcp deep-dive — board config, aliases, multi-port reads, pattern waiting, all tools with examples |
| [`doc/user-manual-jtag-mcp.md`](doc/user-manual-jtag-mcp.md) | jtag-mcp deep-dive — multi-board probe selection, target config, architecture awareness, all 14 tools with examples |
